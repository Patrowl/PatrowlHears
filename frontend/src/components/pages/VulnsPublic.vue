...
export default {
  ...
  methods: {
    ...
    getVulns(page, itemsPerPage, sortBy, sortDesc, extra_filters) {
      ...
      this.$api.get('/api/public/vulns/?limit='+itemsPerPage+'&page='+page+'&search='+this.search+'&'+sorted_by+filter_by_date+extra_filters).then(res => {
        this.vulns = res.data;
        this.loading = false;
        return this.vulns;
      }).catch(e => {
    ...
    clickRow(vulnRow) {
      this.$router.push({ 'name': 'VulnDetailsPublic', 'params': { 'vuln_id': vulnRow.id } });
    },
    getBool(b) {
