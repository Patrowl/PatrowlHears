...
class ExploitCountOrderingFilter(OrderingFilter):
    def filter(self, qs, value):
        if value is not None and "exploit_count" in value:
            qs = (
                qs.prefetch_related("exploitmetadata_set", "orgexploitmetadata_set")
                .annotate(
                    exploit_count=Count("exploitmetadata") + Count("orgexploitmetadata")
                )
                .order_by("exploit_count")
            )
...
class VulnFilter(FilterSet):
    ...
    exploit_count__gt = NumberFilter(
        method="filter_exploit_count__gt", field_name="exploit_count"
    )
    exploit_count__gte = NumberFilter(
        method="filter_exploit_count__gte", field_name="exploit_count"
    )
    exploit_count__lt = NumberFilter(
        method="filter_exploit_count__lt", field_name="exploit_count"
    )
    exploit_count__lte = NumberFilter(
        method="filter_exploit_count__lte", field_name="exploit_count"
    )
    ...
    product_version = CharFilter(
        method="filter_product_version", field_name="product_version"
    )
    ...
    access_vector = CharFilter(
        method="filter_access_vector", field_name="access_vector"
    )
    access_complexity = CharFilter(
        method="filter_access_complexity", field_name="access_complexity"
    )
    access_authentication = CharFilter(
        method="filter_access_authentication", field_name="access_authentication"
    )
    impact_integrity = CharFilter(
        method="filter_impact_integrity", field_name="impact_integrity"
    )
    impact_availability = CharFilter(
        method="filter_impact_availability", field_name="impact_availability"
    )
    impact_confidentiality = CharFilter(
        method="filter_impact_confidentiality", field_name="impact_confidentiality"
    )

    def filter_exploit_count__gt(self, queryset, name, value):
        queryset = queryset.annotate(exploit_count=Count("exploitmetadata")).filter(
            exploit_count__gte=value
        )
    ...
    def filter_exploit_count__lt(self, queryset, name, value):
        queryset = queryset.annotate(exploit_count=Count("exploitmetadata")).filter(
            exploit_count__lt=value
        )

    def filter_exploit_count__lte(self, queryset, name, value):
        return queryset.annotate(exploit_count=Count("exploitmetadata")).filter(
            exploit_count__lte=value
        )
    ...
    def filter_cpe(self, queryset, name, value):
        try:
            ...
            f = {
                ...
                "vulnerable_product_versions__{}__{}__contains".format(
                    vendor, product
                ): version,
            }
            return (
                queryset.prefetch_related("products", "products__vendor")
                .filter(**f)
                .distinct()
            )
        except Exception:
    ...
    def filter_product_version(self, queryset, name, value):
        ...
        if "product_name" in self.data.keys() and "vendor_name" in self.data.keys():
            f = {
                "vulnerable_product_versions__{}__{}__contains".format(
                    ...
                    self.data["product_name"].lower().replace(" ", "_"),
                ): value
            }
            return (
                queryset.prefetch_related("products", "products__vendor")
        ...
        return queryset.filter(
            vulnerable_product_versions__all__contains=value
        ).distinct()
    ...
    sorted_by = ExploitCountOrderingFilter(
        choices=(
            ...
            ("score", _("Score")),
            ("-score", _("Score (Desc)")),
            ("exploit_count", _("NB Exploits")),
            ("-exploit_count", _("NB Exploits (Desc)")),
            ("summary", _("Summary")),
            ...
            ("is_exploitable", _("Exploitable")),
            ("-is_exploitable", _("Exploitable (Desc)")),
            ("is_in_the_news", _("Is in the News")),
            ("-is_in_the_news", _("Is in the News (Desc)")),
            ("is_confirmed", _("Confirmed")),
            ("-is_confirmed", _("Not confirmed")),
        )
    )
...
class ExploitMetadataSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_products(self, instance):
        return [
            {"id": p.id, "name": p.name, "vendor": p.vendor.name}
            for p in instance.vuln.products.all()
        ]
