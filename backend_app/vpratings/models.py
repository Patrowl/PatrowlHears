...
class VPRating(models.Model):
    ...
    def calc_score(self, org=None):
        if not self.data:   # Empty data
            ...
            vpr_threat = self._calc_vpr_threat(org=org)
    ...
    def _calc_vpr_vuln(self):
        ...
        # print("vpr_vuln_score2:", vpr_vuln_score)
        # Remediation
        ...
        # print("vpr_vuln_score3:", vpr_vuln_score)
        # Age
        if 'published' in self.data['vulnerability'].keys():
            for c in VPR_METRICS['vulnerability']['age']['caps'].keys():
                ...
                delta = datetime.now() - pubdate.replace(tzinfo=None)
                if delta.days <= int(c):
    ...
    def _calc_vpr_threat(self, org=None):
        ...
        for exploit in self.vuln.orgexploitmetadata_set.filter(organization=org):
            exploit_score = 0
        ...
        # print("vpr_threat_score1:", vpr_threat_score)
        ...
        # print("vpr_threat_score2:", vpr_threat_score)
        # Check defaut value
        if vpr_threat_score < VPR_DEFAULT_SCORES['threat']:
            vpr_threat_score = VPR_DEFAULT_SCORES['threat']
        # print("vpr_threat_score3:", vpr_threat_score)
        # Cap maximum value
        if vpr_threat_score > VPR_METRICS['threat']['max_score']:
            vpr_threat_score = VPR_METRICS['threat']['max_score']
        # print("vpr_threat_score4:", vpr_threat_score)
        vpr_threat_score = float("{0:.1f}".format(vpr_threat_score))
        # print("vpr_threat_score5:", vpr_threat_score)
    ...
    def _calc_vpr_asset(self):
        ...
        vpr_asset_score = float("{0:.1f}".format(vpr_asset_score))
        # print("vpr_asset_score-5:", vpr_asset_score)
